...
var mergeSort = function(array) {
	//break the the array down into single digit sub-arrays [4,7,4,3,9,1,2] -> [[4],[7],[4],[3],[9],[1],[2]]
	var list = [];
	...
	//merge function to combine adjacent pairs in the form of [[4],[7],[4],[3],[9],[1],[2]] -> [[4,7],[3,4],[1,9],[2]]
	var merge = function(arr) {
		var results = [];
		if(arr.length % 2 === 0) { //for even lengths
			for(var i=0; i<arr.length; i = i+2) {
				//this execution code is for a given pair
				var mergedPair = [];
				while((arr[i].length + arr[i+1].length) !== 0) {
					//if one of the pairs is empty, then shift from the other pair
					if(arr[i].length === 0) {
						var item = arr[i+1].shift();
						mergedPair.push(item);
					} else if(arr[i+1].length === 0) {
						var item = arr[i].shift();
						mergedPair.push(item);
					}
					//if the first index of one of the pairs is less than the other, then shift that index
				...
				results.push(mergedPair);
			}
		} else { //for odd lengths
			for(var i=0; i<(arr.length-1); i = i+2) { //account for the odd length
				//this execution code is for a given pair
				var mergedPair = [];
				while((arr[i].length + arr[i+1].length) !== 0) {
					//if one of the pairs is empty, then shift from the other pair
			...
			results.push(arr.pop()); //account for the odd length - the last element
		}
		return results;
	};

	while(list.length > 1) { //keep merging until the length === 1
		list = merge(list);
	}
